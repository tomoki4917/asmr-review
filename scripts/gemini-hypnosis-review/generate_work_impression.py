...
def main() -> None:
    ...
    if args.slug == "sound-of-ecstasy-saimin":
        extra_note += (
            ...
            "連続ドライ絶頂・解除の体感を中心に。\n"
            "【文体・必須】**解析室レビュアーの所感**（サイト公開向け）。"
            "日記調の「私は〜」連発・口語（パチンと・驚きです）は避け、"
            "usotsuki / hypno-multi-rape 見本の温度（場面→肝→手触り→向く人）。"
            "主観は「〜でした」「〜と感じました」「印象的」等で全体1〜2回。"
            "「購入者」「人も多いようです」「聴き手は」「味わえます」等の"
            "第三者・カタログ調は禁止。DLsiteメモは内部参考のみ。\n"
        )
    extra_note += "\n"
    if args.note.strip():
