...
- 感度Lvカードの HTML は **`kuchikou-saimin-count-trip-nouiki/index.md` からタグ構造のみコピー**（閉じタグは `</div>`。**他記事の本文・パート解説は参照禁止** — `review-no-past-article-reference.mdc`）
...
- `### 梱包ファイル` … 公式ファイル名 … **MM:SS**（合計行なし。**`### パートの長さ` は置かない** — 執筆ガイド §6.1.1）
